...
// Load Libraries
#include <SDI12.h>           // Load the library
...
void setup(){
  ...
  Serial.println();
  Serial.println("Time Elapsed (s), Sensor Address and ID, Volumetric Water Content, Soil Temperature, Bulk Relative Permittivity, Soil Electric Conductivity, Soil Pore Water EC, ... etc.");
  Serial.println("-------------------------------------------------------------------------------");
}
...
void loop(){
  ...
  delay(10000); // (900000 15 MINUTES - wait ten seconds between measurement attempts). // 900000, wait 15 minutes (15 * 60 * 1000 milliseconds)
